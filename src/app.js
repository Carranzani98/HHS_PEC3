window.addEventListener('load', () => {
    ...
    navButton.addEventListener('click', () => {
        ...
        navItems.forEach((navItem, index) => {
            if(index != -1){
                navItem.classList.toggle('responsive-hidden');
            }
        });
    });
    ...
    if(categoryPersonalTab){
        categoryPersonalTab.addEventListener('click', () => {
    ...
    if(categoryExtrasTab){
        categoryExtrasTab.addEventListener('click', () => {
    ...
    if(categoryFilmsTab){
        categoryFilmsTab.addEventListener('click', () => {
            categoryPersonalTab.classList.remove('selected');
            categoryExtrasTab.classList.remove('selected');
...
window.addEventListener('click', e => {
    ...
    navItems.forEach((navItem, index) => {
        if(index != -1){
            navItem.classList.add('responsive-hidden');
        }
    });
});
